function doPost(e) {
  ...
  // 6. XÓA TÀI KHOẢN VÀ LỊCH SỬ GIAO DỊCH
  if (action === "DELETE_ACCOUNT") {
    var tkRows = sheetTaiKhoan.getDataRange().getValues();
    var userRowIndex = -1;
    
    // Tìm ID trong sheet tài khoản
    for (var i = 1; i < tkRows.length; i++) {
      if (tkRows[i][0] == id) {
        userRowIndex = i;
        break;
    ...
    // Kiểm tra mật khẩu (đã băm SHA256)
    if (tkRows[userRowIndex][1] !== data.pass) return respond({status: "error", message: "Mật khẩu không chính xác!"});

    // Xóa dòng tài khoản (Cộng 1 vì mảng chạy từ 0, sheet chạy từ 1)
    sheetTaiKhoan.deleteRow(userRowIndex + 1);

    // Xóa toàn bộ dữ liệu lịch sử bên sheet 'data'
    var dataRows = sheetData.getDataRange().getValues();
    
    // LƯU Ý: Phải quét từ dưới lên trên (k--) khi xóa dòng.
    // Nếu quét từ trên xuống, việc xóa dòng sẽ làm tụt các dòng bên dưới lên, gây sai lệch Index.
    for (var k = dataRows.length - 1; k > 0; k--) {
      if (dataRows[k][7] == id) { // Cột H là ID (Index 7)
        sheetData.deleteRow(k + 1);
      }
    }

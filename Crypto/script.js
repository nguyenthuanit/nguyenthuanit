...
// Cập nhật fetchCloudBot mỗi 3 giây
setInterval(fetchCloudBot, 3000);

// Sửa lại logic thao túng trong simulateMarket
if (isBoRunning) {
    // Ưu tiên hướng đi từ Cloud Bot (ô Z1) nếu có
    const targetDirection = cloudBotDirection || activeAIPrediction;
    if (targetDirection) {
        const isAIHit = Math.random() <= 0.95; // 95% nến đi theo Bot

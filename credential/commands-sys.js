...
Object.assign(commands, {
    ...
    // CẢI TIẾN: Sửa lệnh theme tác động vào #terminalWindow thay vì document.body
    theme: (args) => { 
        const [action, themeName] = args; 
        if (action === 'list') return Object.entries(availableThemes).map(([n, d]) => `  - ${n}: ${d}`).join('\n'); 
        if (action === 'set' && availableThemes[themeName]) { 
            const win = document.getElementById('terminalWindow');
            if (win) win.className = `os-window terminal-window theme-${themeName}`; 
            return `Theme changed to ${themeName}.`; 
        } 
        return "Usage: theme [list|set] [theme_name]"; 
    },

document.addEventListener('DOMContentLoaded', () => {
    ...
    const WindowStateManager = {
        saveState(id, rect) {
            try { localStorage.setItem(`windowState_${id}`, JSON.stringify(rect)); } 
            catch (e) { console.error("Could not save window state:", e); }
        },
        loadState(id) {
            try { const state = localStorage.getItem(`windowState_${id}`); return state ? JSON.parse(state) : null; } 
    ...
    const PopupManager = {
        ...
        show(title, message, type = 'info') {
            ...
            if (type === 'error') { this.icon.innerHTML = '<i class="fa-solid fa-circle-xmark"></i>'; this.icon.classList.add('icon-error'); } 
            else if (type === 'confirm') { this.icon.innerHTML = '<i class="fa-solid fa-circle-question"></i>'; this.icon.classList.add('icon-confirm'); this.cancelBtn.style.display = 'inline-block'; } 
            else { this.icon.innerHTML = '<i class="fa-solid fa-circle-info"></i>'; this.icon.classList.add('icon-info'); }
            this.popup.style.display = 'flex';
        },
        hide() { this.popup.style.display = 'none'; },
        alert(title, message, type = 'info') {
            this.show(title, message, type);
            return new Promise(resolve => { this.okBtn.onclick = () => { this.hide(); resolve(true); }; });
        },
    ...
    // === TERMINAL LOGIC ===
    const Terminal = {
        window: document.getElementById('terminalWindow'), output: document.getElementById('terminalOutput'), input: document.getElementById('terminalInput'),
        closeBtn: document.getElementById('closeTerminalBtn'), isRunning: false, isLoggedIn: false, passwordAttempts: 0,
        onCompleteCallback: null, passwordHandler: null, menuHandler: null,
        CORRECT_PASSWORD_HASH: '8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92', // password is "password"
        preAuthLines: [ { text: 'Booting system...', delay: 500 }, { text: 'Connecting to server //103.77.201.52...', delay: 900 }, { text: 'Connection established.', delay: 500 }, { text: 'Authenticating user: admin', delay: 800 }, ],
        postAuthLines: [ { text: '[OK] ACCESS GRANTED.', delay: 500 }, { text: 'Loading user profile...', delay: 800 }, ],
        menuText: [ { text: '\nWelcome, admin. Please select an option:' }, { text: '(1) File Explorer' }, { text: '(2) System Diagnostics' }, { text: '(3) Network Configuration' }, { text: '(4) Security Logs' }, { text: '(5) Music Player' }, { text: '(6) Mail Client' }, { text: '(7) Help' } ],
        init() {
        ...
        handlePasswordSubmit(event) {
            ...
            if(sha256(pass) === this.CORRECT_PASSWORD_HASH) { this.isLoggedIn = true; this.runPostAuth(); } 
            else {
                this.passwordAttempts++;
                if (this.passwordAttempts >= 3) { this.output.innerHTML += `<span class="line error">Authentication failed. Maximum attempts reached.</span>`; this.isRunning = false; } 
                else { this.output.innerHTML += `<span class="line error">Sorry, try again.</span>`; this.promptForPassword(); }
            }
        },
        async runPostAuth() {
            await this.printLines(this.postAuthLines);
            if (this.isRunning) { if (this.onCompleteCallback) this.onCompleteCallback(); this.showMenu(); }
        },
        ...
        async run(onComplete) {
            ...
            this.isRunning = true; this.isLoggedIn = false; this.passwordAttempts = 0; this.onCompleteCallback = onComplete;
            this.output.innerHTML = ''; this.show();
    ...
    const FileExplorer = {
        ...
        addressInput: document.getElementById('addressInput'), contextMenu: document.getElementById('contextMenu'), newFolderAction: document.getElementById('newFolderAction'), deleteAction: document.getElementById('deleteAction'),
        ...
        async init() {
            ...
            try { await DBManager.init(); const folders = await DBManager.getAllFolders(); if (folders.length === 0) { for (const folder of this.fixedFolders) await DBManager.saveFolder(folder); } } 
            catch (e) { PopupManager.alert('Database Error', 'Could not initialize the file system.', 'error'); return; }
            this.setupEventListeners(); this.render();
            dragElement(this.explorerWindow, this.explorerTitleBar); makeResizable(this.explorerWindow);
            const savedState = WindowStateManager.loadState('explorerWindow');
            if (savedState) { this.explorerWindow.style.top = savedState.top; this.explorerWindow.style.left = savedState.left; this.explorerWindow.style.width = savedState.width; this.explorerWindow.style.height = savedState.height; }
        ...
        async render() {
            ...
            try {
                if (this.state.currentView === 'folders') {
                    this.explorerBody.classList.remove('list-view'); this.backButton.style.display = 'none'; this.addFileBtn.style.display = 'none';
                    const folders = await DBManager.getAllFolders();
                    this.explorerBody.innerHTML = ''; folders.forEach(folder => this.explorerBody.appendChild(this.createFolderElement(folder)));
                } else if (this.state.currentView === 'files') {
                    this.explorerBody.classList.add('list-view'); this.backButton.style.display = 'block'; this.addFileBtn.style.display = 'block';
                    const files = await DBManager.getFilesByFolder(this.state.currentFolder.id);
                    this.explorerBody.innerHTML = ''; this.explorerBody.appendChild(this.createListViewHeader()); files.forEach(file => this.explorerBody.appendChild(this.createFileElement(file)));
                }
            } catch (e) { this.explorerBody.innerHTML = '<p style="padding: 20px; text-align: center;">Error loading items.</p>'; PopupManager.alert('File System Error', 'Could not retrieve items.', 'error'); }
        },
        ...
        createFolderElement(folder) { const el = document.createElement('div'); el.className = 'grid-item folder-item'; el.dataset.id = folder.id; el.dataset.name = folder.name; el.dataset.type = 'folder'; el.dataset.deletable = folder.isDeletable || false; el.innerHTML = `<i class="${folder.icon === 'desktop' ? 'fa-solid fa-desktop' : 'fa-solid fa-folder'}"></i><span>${folder.name}</span>`; return el; },
        createFileElement(file) { const el = document.createElement('div'); el.className = 'grid-item file-item'; el.dataset.id = file.id; el.dataset.type = 'file'; el.innerHTML = `<div class="item-col col-name"><i class="${this.getIconForFileType(file.fileType)}"></i><span>${file.fileName}</span></div><div class="item-col col-date"><span>${this.formatDate(file.lastModified)}</span></div><div class="item-col col-size"><span>${this.formatBytes(file.fileSize)}</span></div>`; return el; },
        ...
        getIconForFileType(fileType) { if (fileType.startsWith('image/')) return 'fa-solid fa-file-image'; if (fileType.includes('pdf')) return 'fa-solid fa-file-pdf'; if (fileType.includes('word')) return 'fa-solid fa-file-word'; return 'fa-solid fa-file-lines'; },
        ...
        handleMaximize() {
            if (this.state.isMaximized) {
                ...
                this.explorerWindow.style.top = top; this.explorerWindow.style.left = left; this.explorerWindow.style.width = width; this.explorerWindow.style.height = height;
        ...
        handleClick(e) {
            ...
            if (e.detail === 2) { 
                if (type === 'folder') { this.state.currentView = 'files'; this.state.currentFolder = { id, name }; this.render(); } 
                else if (type === 'file') { DBManager.db.transaction('files').objectStore('files').get(parseInt(id)).onsuccess = e => { if(e.target.result) this.showPreviewModal(e.target.result); }; }
            }
        },
        ...
        handleRightClick(e) {
            e.preventDefault();
            const item = e.target.closest('.grid-item');
            this.selectItem(item); this.newFolderAction.style.display = 'none'; this.deleteAction.style.display = 'none';
            if (item) { if (item.dataset.deletable === 'true' || item.dataset.type === 'file') { this.state.currentItemForAction = { id: item.dataset.id, type: item.dataset.type }; this.deleteAction.style.display = 'flex'; } } 
            else if (e.target.closest('.explorer-body') && this.state.currentView === 'folders') { this.newFolderAction.style.display = 'flex'; } 
            else { return; }
            this.contextMenu.style.top = `${e.clientY}px`; this.contextMenu.style.left = `${e.clientX}px`; this.contextMenu.style.display = 'block';
        },
        async handleNewFolder() { const folderName = prompt('Nhập tên thư mục mới:'); if (folderName) { try { await DBManager.saveFolder({ id: `user-folder-${Date.now()}`, name: folderName, icon: 'desktop', isDeletable: true }); this.render(); } catch(e) { PopupManager.alert('Error', 'Could not create new folder.', 'error'); } } },
        async handleFileUpload(e) { if (e.target.files.length > 0 && this.state.currentFolder) { try { await DBManager.saveFile(e.target.files[0], this.state.currentFolder.id); this.render(); } catch (e) { PopupManager.alert('Error', 'Could not save the file.', 'error'); } finally { this.fileUploadInput.value = null; } } },
        async handleDelete() {
            const { id, type } = this.state.currentItemForAction; if (!id || !type) return;
            const confirmed = await PopupManager.confirm('Xác nhận xóa', `Bạn có chắc muốn xóa vĩnh viễn ${type} này?`);
            if (confirmed) { try { if (type === 'folder') await DBManager.deleteFolder(id); else if (type === 'file') await DBManager.deleteFile(parseInt(id)); this.render(); } catch (e) { PopupManager.alert('Error', `Could not delete the ${type}.`, 'error'); } }
        ...
        showPreviewModal(fileRecord) {
            ...
            if (fileType.startsWith('image/')) { const img = document.createElement('img'); img.src = fileURL; this.filePreviewContent.appendChild(img); } 
            else if (fileType.startsWith('text/') || fileType === 'application/json') { const reader = new FileReader(); reader.onload = (e) => { const pre = document.createElement('pre'); pre.textContent = e.target.result; this.filePreviewContent.appendChild(pre); }; reader.readAsText(fileData); } 
            else { this.filePreviewContent.textContent = `Không thể xem trước file '${fileName}'.`; }
    ...
    const MailClient = {
        ...
        sidebar: document.getElementById('mailSidebar'), content: document.getElementById('mailContent'),
        closeBtn: document.getElementById('closeMailBtn'), minimizeBtn: document.getElementById('minimizeMailBtn'),
        maximizeBtn: document.getElementById('maximizeMailBtn'), maximizeIcon: document.getElementById('maximizeMailIcon'),
        restoreIcon: document.getElementById('restoreMailIcon'), newMailBtn: document.getElementById('newMailBtn'),
        state: { isMaximized: false, lastPosition: {} },
        fakeEmails: [
            { id: 1, sender: 'System Security', subject: 'Security Alert: New sign-in', timestamp: '2025-10-08T18:15:00', body: '<p>Dear Admin,</p><p>We detected a new sign-in to your account from a simulated device. If this was you, you can safely ignore this email.</p><p>Thank you,<br/>System Security Team</p>' },
            { id: 2, sender: 'HR Department', subject: 'Welcome to the Team!', timestamp: '2025-10-08T14:30:00', body: '<p>Welcome aboard!</p><p>We are thrilled to have you join our team. Your account has been set up and you can now access all system applications.</p><p>Best regards,<br/>The HR Team</p>' },
            { id: 3, sender: 'IT Support', subject: 'Your access credentials', timestamp: '2025-10-08T14:25:00', body: '<p>Hello Admin,</p><p>Your system access has been granted. Please remember that for security reasons, sending emails from this client is disabled.</p><p>For any assistance, please contact IT Support.</p>' },
            { id: 4, sender: 'Project Manager', subject: 'Project Phoenix Kick-off', timestamp: '2025-10-07T11:00:00', body: '<p>Hi Team,</p><p>This is a reminder about the kick-off meeting for Project Phoenix tomorrow. Please be prepared to discuss the initial phase deliverables.</p><p>Thanks!</p>' }
        ],
        init() {
            if (this.isInitialized) return;
            dragElement(this.window, this.titleBar); makeResizable(this.window);
            this.setupEventListeners(); this.renderMailList(); this.renderMailContent(1);
            const savedState = WindowStateManager.loadState('mailClientWindow');
            if (savedState) { this.window.style.top = savedState.top; this.window.style.left = savedState.left; this.window.style.width = savedState.width; this.window.style.height = savedState.height; }
            this.isInitialized = true;
        },
        setupEventListeners() {
            ...
            this.newMailBtn.addEventListener('click', () => { PopupManager.alert('Permission Denied', 'User is not permitted to send messages.', 'error'); });
        },
        renderMailList() {
            this.sidebar.innerHTML = '';
            this.fakeEmails.forEach(email => {
                const item = document.createElement('div'); item.className = 'mail-list-item'; item.dataset.id = email.id;
                item.innerHTML = `<div class="mail-sender">${email.sender}</div><div class="mail-subject">${email.subject}</div>`;
                item.addEventListener('click', () => this.renderMailContent(email.id));
                this.sidebar.appendChild(item);
            });
        },
        renderMailContent(emailId) {
            const email = this.fakeEmails.find(e => e.id === emailId);
            if (!email) { this.content.innerHTML = `<div class="no-mail-selected"><i class="fa-regular fa-envelope"></i><p>Select an item to read</p></div>`; return; }
            this.content.innerHTML = `<div class="mail-header"><h2 class="mail-content-subject">${email.subject}</h2><div class="mail-content-sender"><strong>${email.sender}</strong> <span>- ${new Date(email.timestamp).toLocaleString()}</span></div></div><div class="mail-content-body">${email.body}</div>`;
            this.sidebar.querySelectorAll('.mail-list-item').forEach(item => { item.classList.toggle('active', item.dataset.id == emailId); });
        },
        handleMaximize() {
            if (this.state.isMaximized) {
                this.window.classList.remove('maximized');
                const { top, left, width, height } = this.state.lastPosition;
                this.window.style.top = top; this.window.style.left = left; this.window.style.width = width; this.window.style.height = height;
                this.maximizeIcon.style.display = 'block'; this.restoreIcon.style.display = 'none';
            } else {
    ...
    function setupDock() {
        ...
        dockMailClient.classList.add('disabled');
    }
    setupDock();
    ...
    function dragElement(elmnt, header) {
        ...
        function dragMouseDown(e){ if (elmnt.classList.contains('maximized')) return; e=e||window.event; e.preventDefault(); p3=e.clientX; p4=e.clientY; document.onmouseup=closeDragElement; document.onmousemove=elementDrag; }
    ...
    function makeResizable(element) {
        element.querySelectorAll('.resizer').forEach(resizer => {
            resizer.addEventListener('mousedown', (e) => {
                ...
                const original_w = rect.width, original_h = rect.height;
                const original_x = e.clientX, original_y = e.clientY;
                const original_left = rect.left, original_top = rect.top;
    ...
    window.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            if (document.getElementById('filePreviewModal').style.display === 'flex') FileExplorer.hidePreviewModal();
            else if (document.getElementById('winPopup').style.display === 'flex') PopupManager.hide();
        }
    });
});
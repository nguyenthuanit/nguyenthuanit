/**
 * NMT VIP BO - BACKEND GOOGLE APPS SCRIPT (VERSION FULL - CHỐNG MẤT CODE)
 * Hỗ trợ: Đồng bộ VIP qua Volume, Đăng ký, Đăng nhập, Giao dịch, Bot AI, Ngân hàng trung tâm.
 */
...
function doPost(e) {
  ...
  try {
    ...
    // --- LẤY SỐ DƯ, LỊCH SỬ & ĐỒNG BỘ VIP (GET_BALANCE) ---
    if (action === "GET_BALANCE") {
      var dataRows = sheetData.getDataRange().getValues();
      var currentBalance = 0; 
      var userHistory = [];
      var totalVolume = 0; // Quan trọng: Dùng để đồng bộ VIP qua thiết bị khác
      var foundBalance = false;

      // Quét từ dưới lên để lấy số dư mới nhất và tính tổng Volume
      for (var j = dataRows.length - 1; j > 0; j--) {
        if (dataRows[j][7] == id) {
          // Tính tổng cược (Volume)
          var act = dataRows[j][1] || "";
          if (act.indexOf("Cược") > -1) {
            totalVolume += parseFloat(dataRows[j][2]) || 0;
          }
      ...
      // Trả về cả totalVolume để client tính cấp VIP
      return respond({status: "success", balance: currentBalance, history: userHistory, dotHistory: savedDots, totalVolume: totalVolume});
    }
